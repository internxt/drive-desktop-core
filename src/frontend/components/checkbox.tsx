...
export function Checkbox(props: CheckboxProps) {
  ...
  const checkboxStateClasses = props.checked
  ...
  return (
    <label
      className={`group flex items-start space-x-2 text-base leading-5 ${
        isDisabled ? 'text-gray-40 cursor-not-allowed' : 'text-gray-100'
      ...
      <div className="relative h-5 w-5">
        <input
          ...
          className={`h-5 w-5 shrink-0 appearance-none rounded-md border border-gray-30 shadow-sm transition-all duration-75 ease-in-out ${
            isDisabled ? 'cursor-not-allowed opacity-50 bg-gray-10 border-gray-20' : `cursor-pointer ${checkboxStateClasses}`
          }`}
          onClick={(e) => {
            e.preventDefault();
            if (!isDisabled) {
