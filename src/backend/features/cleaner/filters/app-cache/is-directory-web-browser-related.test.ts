...
describe('isDirectoryWebBrowserRelated', () => {
  it.each(['google-chrome', 'chromium', 'firefox', 'opera'])('should return true for exact browser directory names: %s', (name) => {
    expect(isDirectoryWebBrowserRelated({ folderName: name })).toBe(true);
  });

  it.each(['Google-Chrome', 'CHROMIUM', 'Firefox', 'OPERA'])('should return true for browser names with case variations: %s', (name) => {
    expect(isDirectoryWebBrowserRelated({ folderName: name })).toBe(true);
  });
  ...
  it.each(['vscode', 'telegram', 'discord', 'spotify'])('should return false for non-browser directory names: %s', (name) => {
    expect(isDirectoryWebBrowserRelated({ folderName: name })).toBe(false);
  });
